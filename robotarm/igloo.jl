...
tmp = collect(linspace(0.0,360.0,73));
...
for i=1:nb_elem
	...
	if (abs(sin(deg2rad(theta[i])))>0.01)
		...
		phi_step = maximum(tab_phi_step[find(tab_phi_step.<=phi_step)]);
	else
		phi_step = 60.0;
	end
	...
	tab_phi = mod.(phi_shift+collect(Float64,linspace(0.0,360.0,nb_phi)),360.0);
...
itmp = indmin(abs.(tab_increment_theta-theta_step));
...
#		phi_step = 2*rad2deg(asin(2*sin(deg2rad(theta_step*0.5))/(sqrt(3)*sin(deg2rad(theta[i])))));
		itmp = indmin(abs.(ntab_phi_step-phi_step));
